...
filename_id_hybridCut  = "seed%d_noise%d_signal%d_digitization%d" % (args.seedEt_thresh, args.noise_filter, args.tower_thresh, args.digitization)

filename_noNoiseCut    = "../ttbar/data/seed%d/leading_jets_%s.pkl" % (args.seedEt_thresh, filename_id_noNoiseCut)
filename_wNoiseCut     = "../ttbar/data/seed%d/leading_jets_%s.pkl" % (args.seedEt_thresh, filename_id_wNoiseCut)
filename_hybridCut     = "data/seed%d/leading_jets_%s.pkl" % (args.seedEt_thresh, filename_id_hybridCut)
...
data = pickle.load(file(filename_hybridCut))
oJets_hybridCut = []
tJets_hybridCut = []
rho_hybridCut   = []
for oJet,tJet,rho in zip(data['leading_offline_jet'], data['matched_trigger_jet'], data['gFEX_rho_all']):
  if abs(oJet.eta) > 1.6:
    continue
  oJets_hybridCut.append( (oJet.m, oJet.Pt, oJet.nsj, oJet.subjetsPt) )
  tJets_hybridCut.append( (tJet.Et, tJet.area) )
  rho_hybridCut.append( rho )
oJets_hybridCut = np.array(oJets_hybridCut, dtype=oJet_dtype)
tJets_hybridCut = np.array(tJets_hybridCut, dtype=tJet_dtype)
rho_hybridCut   = np.array(rho_hybridCut, dtype=np.float)
del data
...
for oJet_subjetPt_cut, oJet_nsj_cut in zip(offline_subjetPt_thresholds, offline_nsj_thresholds):
  ...
  subjets_cut_hybridCut[oJet_nsj_cut] = np.array(map(lambda x: custom_filter(x, oJet_subjetPt_cut, oJet_nsj_cut), oJets_hybridCut['subjets']))

#will return a set of truth values
def range_cut(items, range):
  return (range[0] <= items)&(items <= range[1])

mass_cut1_noNoiseCut = range_cut(oJets_noNoiseCut['m'], (50.,100.) )
mass_cut1_wNoiseCut  = range_cut(oJets_wNoiseCut['m'], (50.,100.) )
mass_cut1_hybridCut  = range_cut(oJets_hybridCut['m'], (50.,100.) )

mass_cut2_noNoiseCut = range_cut(oJets_noNoiseCut['m'], (100.,220.) )
mass_cut2_wNoiseCut  = range_cut(oJets_wNoiseCut['m'], (100.,220.) )
mass_cut2_hybridCut  = range_cut(oJets_hybridCut['m'], (100.,220.) )

offline_cuts_noNoiseCut = [\
  subjets_cut_noNoiseCut[1],\
  subjets_cut_noNoiseCut[1]&mass_cut1_noNoiseCut,\
  subjets_cut_noNoiseCut[2]&mass_cut1_noNoiseCut,\
  subjets_cut_noNoiseCut[3]&mass_cut1_noNoiseCut,\
  subjets_cut_noNoiseCut[1]&mass_cut2_noNoiseCut,\
  subjets_cut_noNoiseCut[2]&mass_cut2_noNoiseCut,\
  subjets_cut_noNoiseCut[3]&mass_cut2_noNoiseCut]

offline_cuts_wNoiseCut = [\
  subjets_cut_wNoiseCut[1],\
  subjets_cut_wNoiseCut[1]&mass_cut1_wNoiseCut,\
  subjets_cut_wNoiseCut[2]&mass_cut1_wNoiseCut,\
  subjets_cut_wNoiseCut[3]&mass_cut1_wNoiseCut,\
  subjets_cut_wNoiseCut[1]&mass_cut2_wNoiseCut,\
  subjets_cut_wNoiseCut[2]&mass_cut2_wNoiseCut,\
  subjets_cut_wNoiseCut[3]&mass_cut2_wNoiseCut]

offline_cuts_hybridCut = [\
  subjets_cut_hybridCut[1],\
  subjets_cut_hybridCut[1]&mass_cut1_hybridCut,\
  subjets_cut_hybridCut[2]&mass_cut1_hybridCut,\
  subjets_cut_hybridCut[3]&mass_cut1_hybridCut,\
  subjets_cut_hybridCut[1]&mass_cut2_hybridCut,\
  subjets_cut_hybridCut[2]&mass_cut2_hybridCut,\
  subjets_cut_hybridCut[3]&mass_cut2_hybridCut]
...
def fit_func(x, y):
  try:
    ...
    return -1.
  return popt #returns (a,b,c,d)

PtSubjetStr = 'anti-$K_T\ R=1.0$ (5% trimmed)\n$N\left(P_T^{{D=0.3\ \mathrm{{subjet}}}} > 20\ \mathrm{{GeV}}\\right) \geq {0}$'
MassCut1Str = '$50 \leq m^\mathrm{oJet} \leq 100$ GeV'
MassCut2Str = '$100 \leq m^\mathrm{oJet} \leq 220$ GeV'
EtaCutStr   = '$|\eta^\mathrm{oJet}| \leq 1.6$' 

plotStrings = [\
  '%s\n%s' % (EtaCutStr, PtSubjetStr.format(1) ),\
  '%s\n%s\n%s' % (EtaCutStr, MassCut1Str, PtSubjetStr.format(1) ),\
  '%s\n%s\n%s' % (EtaCutStr, MassCut1Str, PtSubjetStr.format(2) ),\
  '%s\n%s\n%s' % (EtaCutStr, MassCut1Str, PtSubjetStr.format(3) ),\
  '%s\n%s\n%s' % (EtaCutStr, MassCut2Str, PtSubjetStr.format(1) ),\
  '%s\n%s\n%s' % (EtaCutStr, MassCut2Str, PtSubjetStr.format(2) ),\
  '%s\n%s\n%s' % (EtaCutStr, MassCut2Str, PtSubjetStr.format(3) )]

nn = 0
for cuts_noNoiseCut, cuts_wNoiseCut, cuts_hybridCut, plotStr in zip(offline_cuts_noNoiseCut, offline_cuts_wNoiseCut, offline_cuts_hybridCut, plotStrings):

  # prepare the cuts by np.where(Truth-arrays)
  cuts_noNoiseCut = np.where(cuts_noNoiseCut)
  cuts_wNoiseCut  = np.where(cuts_wNoiseCut)
  cuts_hybridCut  = np.where(cuts_hybridCut)
  ...
  trigger_jet_Et_hybridCut_noPileup  = trigger_jet_Et_hybridCut - tJet_Et_correction_hybridCut
  offline_jet_Pt_hybridCut_noPileup  = offline_jet_Pt_hybridCut[np.where(trigger_jet_Et_hybridCut_noPileup > 0.)]
  trigger_jet_Et_hybridCut_noPileup  = trigger_jet_Et_hybridCut_noPileup[np.where(trigger_jet_Et_hybridCut_noPileup > 0.)]
  ...
  hist_efficiency_den = [\
    ...
    np.histogram(offline_jet_Pt_hybridCut_noPileup, bins=bins_efficiency)[0] ]
  ...
  hist_efficiency_num = [\
    ...
    np.histogram(offline_jet_Pt_hybridCut_noPileup[np.where( (trigger_jet_Et_hybridCut_noPileup > tJet_Et_cut) )], bins=bins_efficiency)[0] ]
  ...
  numEvents = [\
    ...
    offline_jet_Pt_hybridCut_noPileup.size ]
  ...
  for den, num, label, numEvent, marker, color in zip(hist_efficiency_den, hist_efficiency_num, labels, numEvents, markers, colors):
    ...
    peak_point = w[1]*erfinv((0.95-w[3])/w[0])+w[2]
    pl.errorbar(xpoints_efficiency[nonzero_bins], hist_eff_curve_diff, linestyle='-', yerr=errors_eff_diff, ecolor='black', marker=marker, c=color, mfc=color, ms=20, mec=color, label='%s\n$w = %0.1f,\ x_{0.95} = %0.1f$' % (label, w[1], peak_point), linewidth=linewidth, alpha=0.75)
