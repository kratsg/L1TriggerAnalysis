...
trigger_jet_Et['all'], trigger_jet_area['all'], trigger_jet_region = np.array([[tJet.Et, tJet.area, tJet.region()] for tJet in data['matched_trigger_jet']]).T
...
for i in [1,2,3,4]:
  ...
  trigger_jet_Et_correction[regionStr] = trigger_jet_area[regionStr]*data['gFEX_rho_%d' % i][region]
...
basicTextStr = {}
for i in [1,2,3,4]:
  basicTextStr['region_%d' % i] = '%s\n%d Events \n%s\n%s\n%s' % (dataSetStr, numEvents['region_%d' % i], seedCutStr, noiseCutStr, towerThrStr)
...
for i in [1,2,3,4]:
  ...
  pl.savefig( write_file('plots/trigger_jet_Et_correction/%s_trigger_jet_Et_correction_region%d.png' % (filename_id, i)) )
  pl.close()
  ...
  pl.grid(True, which='both', linewidth=3, linestyle='--', alpha=0.5)
  pl.tick_params(axis='both', which='major', labelsize=labelsize)
  pl.savefig( write_file('plots/jet_energy_correlation/%s_region%d.png' % (filename_id, i)) )
  pl.close()
  ...
  pl.grid(True, which='both', linewidth=3, linestyle='--', alpha=0.5)
  pl.tick_params(axis='both', which='major', labelsize=labelsize)
  pl.savefig( write_file('plots/jet_energy_correlation/%s_noPileup_region%d.png' % (filename_id, i)) )
  pl.close()
  ...
  pl.grid(True, which='both', linewidth=3, linestyle='--', alpha=0.5)
  pl.tick_params(axis='both', which='major', labelsize=labelsize)
  pl.savefig( write_file('plots/resolution/%s_resolution_PtOffline_region%d.png' % (filename_id, i)) )
  pl.close()
  ...
  pl.grid(True, which='both', linewidth=3, linestyle='--', alpha=0.5)
  pl.tick_params(axis='both', which='major', labelsize=labelsize)
  pl.savefig( write_file('plots/resolution/%s_resolution_PtOffline_noPileup_region%d.png' % (filename_id, i)) )
  pl.close()
  ...
  pickle.dump(pl_res_proj, file( write_file('plots/pickle/%s_resolution_PtOffline_projection_region%d.pkl' % (filename_id, i)), 'w+') )
  pl.savefig( write_file('plots/resolution/%s_resolution_PtOffline_projection_region%d.png' % (filename_id, i)) )
  pl.close()
  ...
  pickle.dump(pl_res_proj, file( write_file('plots/pickle/%s_resolution_PtOffline_projection_noPileup_region%d.pkl' % (filename_id, i)), 'w+') )
  pl.savefig( write_file('plots/resolution/%s_resolution_PtOffline_projection_noPileup_region%d.png' % (filename_id, i)) )
  pl.close()
  ...
  for triggerEt_thresh in [0., 100., 110., 130., 150., 200., 220.]:
    ...
    pickle.dump(pl_eff_diff, file( write_file('plots/pickle/matched_differential_%s_trigger%d_region%d.pkl' % (filename_id, triggerEt_thresh, i)), 'w+') )
    pl.savefig( write_file('plots/differential/%s_trigger%d_region%d.png' % (filename_id, triggerEt_thresh, i)) )
    pl.close()
    ...
    pl_eff_int = {'xdata'          : xpoints_efficiency,\
                  ...
                  'nonzero_bins'   : nonzero_bins,\
                  'eff_curve_shift': eff_curve_shift}
    pickle.dump(pl_eff_diff, file( write_file('plots/pickle/matched_integral_%s_trigger%d_region%d.pkl' % (filename_id, triggerEt_thresh, i)), 'w+') )
    pl.savefig( write_file('plots/integral/%s_trigger%d_region%d.png' % (filename_id, triggerEt_thresh, i)) )
    pl.close()
