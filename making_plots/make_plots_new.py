...
def latex_float(f, pos=0):
    ...
    if "e" in float_str:
        ...
        '''if len(float_str) == 1:
          return r"${0}.0 \times 10^{{{1}}}$".format(float_str, 0)
        elif len(float_str) == 2:
          return r"${0}. \times 10^{{{1}}}$".format(float_str, 0)
        else:
          return r"${0} \times 10^{{{1}}}$".format(float_str, 0)'''
...
hist, _, _, im = ax.hist2d(data['offline_rho'], data['gFEX_rho_1'], norm=LogNorm(), bins=(bins_rho, bins_rho) , alpha=0.75, cmap = cmap)
...
ax.set_xlabel(label_oRho, fontsize=labelsize)
ax.set_ylabel('%s, %s' % (label_gRho, region_legend[1]), fontsize=labelsize)
ax.grid(True, which='both', linewidth=3, linestyle='--', alpha=0.5)
ax.text(0.95, 0.05, '%s\n%s\n$\mathrm{Corr} = %0.4f$' % (dataSetStr, towerThrStr, corr), transform=ax.transAxes, fontsize=labelsize, verticalalignment='bottom', horizontalalignment='right', bbox=textprops)

#add atlas simulation
#    internal
ax.text(0.05, 0.95, 'ATLAS', fontsize=42, style='italic', fontweight='bold', verticalalignment='top', horizontalalignment='left', transform=ax.transAxes)
ax.text(0.27, 0.95, 'Preliminary', verticalalignment='top', horizontalalignment='left', fontsize=40, transform=ax.transAxes)
ax.text(0.05, 0.90, 'Simulation', verticalalignment='top', horizontalalignment='left', fontsize=40, transform=ax.transAxes)

ax.tick_params(axis='both', which='major', labelsize=labelsize-6)
fig.savefig( write_file('ZH/%s_%s_correlation.png' % (filename_id, 'gFEX_rho_1')), bbox_inches='tight')
fig.savefig( write_file('ZH/%s_%s_correlation.eps' % (filename_id, 'gFEX_rho_1')), bbox_inches='tight')
fig.savefig( write_file('ZH/%s_%s_correlation.pdf' % (filename_id, 'gFEX_rho_1')), bbox_inches='tight')
