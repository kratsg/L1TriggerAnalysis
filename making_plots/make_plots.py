...
filename = "ttbar_leading_jets_%s.pkl" % (filename_id)
...
cbar = pl.colorbar(ticks=[0., 1., 2., 5., 10., 20., 50., 100., 200., 500., 1000., 2000., 5000.], format=formatter)
cbar.set_label('number density', fontsize=labelsize, labelpad=40)
...
pl.savefig( write_file('ttbar/%s_%s_correlation.png' % (filename_id, 'gFEX_rho_1')), bbox_inches='tight')
pl.savefig( write_file('ttbar/%s_%s_correlation.eps' % (filename_id, 'gFEX_rho_1')), bbox_inches='tight')
pl.savefig( write_file('ttbar/%s_%s_correlation.pdf' % (filename_id, 'gFEX_rho_1')), bbox_inches='tight')

pickle.dump(fig.gca(), file('ttbar_ax.pkl', 'w+'))
