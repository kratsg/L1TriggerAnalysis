...
for event_num in xrange(args.event_start, args.event_start+args.num_events, args.step_size):
  ...
  for row in data:
    ...
    hist, bins = np.histogram( np.floor(event['gTowerEt'].tolist()/args.digitization)*args.digitization/1000., bins=np.arange(0.0,50.0,1.0) )

    event_data = np.array([(matched_jets[0][0], matched_jets[0][1], offline_rho, gFEX_rho['all'], gFEX_rho[1], gFEX_rho[2], gFEX_rho[3], gFEX_rho[4], num_vertices, hist)], dtype=datatype)
