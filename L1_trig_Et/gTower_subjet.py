...
all_gTower_Et = np.sort(np.append([seed_Et],gTower_Et, axis=0))
...
for datapts, color, label in zip( all_gTower_Et, colors, labels ):
...
for datapts, label in zip(all_gTower_Et, ['gTower seed','gTower 1','gTower 2','gTower 3', 'gTower 4']):
    fig = pl.figure(figsize=figsize)
    counts, edges_x, edges_y, H = pl.hist2d(datapts, offline_jet_nsj, bins=(np.arange(0.,100.,2.5), np.arange(1,7,1)), norm=LogNorm())
    points_y, mean_x = profile_x(edges_y, offline_jet_nsj, datapts)
    profiles[label.replace(' ','_')] = {'x': mean_x, 'y': points_y}
    ...
    points_y, mean_x = profile_x(edges_y, offline_jet_nsj, datapts/trigger_jet_Et)
    profiles_scaled[label.replace(' ','_')] = {'x': mean_x, 'y': points_y}
    pl.scatter(mean_x, points_y, s=markersize, facecolor='w', edgecolor='k', marker='o', linewidth=2)
    cbar = pl.colorbar(H)
    pl.text(0.95, 0.05, basicTextStr, transform=fig.gca().transAxes, fontsize=labelsize, verticalalignment='bottom', horizontalalignment='right', bbox=textprops)
...
#profile_x's merged
fig = pl.figure(figsize=figsize)
for l,c,m in zip(['gTower seed','gTower 1','gTower 2','gTower 3', 'gTower 4'], colors, markers):
  datapts = profiles[l.replace(' ','_')]
  pl.scatter(datapts['x'], datapts['y'], s=markersize, facecolor=c, edgecolor='k', marker=m, linewidth=1, alpha=0.75, label=l)
